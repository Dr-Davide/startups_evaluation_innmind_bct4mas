 ...
 *
 */
public class LoadFeatureListFromLedgerStartup extends OneShotBehaviour {

    private static final Logger log = Logger.getLogger(LoadFeatureListFromLedgerStartup.class);


    private static final long serialVersionUID = 2667054425771882233L;
    BCAgent bcAgent = null;

    public LoadFeatureListFromLedgerStartup(BCAgent a) {
        super(a);
        bcAgent = a;
    }
    ...
    public void action() {
        ...
        try {
            log.info("Agent " + bcAgent.getLocalName()
                    + " getting his services from the ledger (persistence) STARTUP VIEW WOOOW");
            ...
            if (CheckerController.isAgentAlreadyInLedger(bcAgent)) {
                ...
                agentFeaturesList = RangeQueriesController.getStructFeatureListProvidedByAgent(
                        bcAgent.getHfClient(), bcAgent.getHfTransactionChannel(), agentId, InnMindReputation.STARTUP_ROLE);
