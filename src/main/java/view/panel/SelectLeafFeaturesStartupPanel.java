...
public class SelectLeafFeaturesStartupPanel extends JPanel {
     ...
     * Create the panel.
     */
    SelectLeafFeaturesStartupPanel() {

        String[] columnTitles = {"Selected", "Id", "Name", "InnMindReputation"};
    ...
    private Object[][] fillTableModelData(ArrayList<FeatureView> featureViewList) {
        ...
        Object[][] data = new Object[leafFeatures][4];
        int j = 0;
        for (int i = 0; i < row; i++) {
            if (featureViewList.get(i).getFeatureComposition().isEmpty()) {
                ...
                data[j][3] = featureViewList.get(i).getReputation();
                j++;
            }
        }
        return data;
