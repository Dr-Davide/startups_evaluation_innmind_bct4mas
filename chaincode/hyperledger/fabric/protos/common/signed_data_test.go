...
func TestConfigEnvelopeAsSignedData(t *testing.T) {
	...
	for aclMessage := range configSignatures {
		configSignatures[aclMessage] = &ConfigSignature{
			SignatureHeader: marshalOrPanic(&SignatureHeader{
				Creator: identities[aclMessage],
			}),
			Signature: signatures[aclMessage],
		}
	}
	...
	for aclMessage, sigData := range signedData {
		if !bytes.Equal(sigData.Identity, identities[aclMessage]) {
			t.Errorf("Expected identity to match at index %d", aclMessage)
		}
		if !bytes.Equal(sigData.Data, append(configSignatures[aclMessage].SignatureHeader, configBytes...)) {
			t.Errorf("Expected signature over concatenation of config item bytes and signature header")
		}
		if !bytes.Equal(sigData.Signature, signatures[aclMessage]) {
			t.Errorf("Expected signature to match at index %d", aclMessage)
		}
	}
}
