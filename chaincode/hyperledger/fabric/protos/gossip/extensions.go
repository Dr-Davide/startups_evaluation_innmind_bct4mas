...
func (m *SignedGossipMessage) String() string {
	...
	if m.GossipMessage != nil {
		...
		if !isSimpleMsg {
			description := fmt.Sprintf("Channel: %s, nonce: %d, tag: %s", string(m.Channel), m.Nonce, GossipMessage_Tag_name[int32(m.Tag)])
			gMsg = fmt.Sprintf("%s %s", description, gMsg)
		}
	}
...
func (res *RemotePvtDataResponse) ToString() string {
	...
	for aclMessage, el := range res.Elements {
		a[aclMessage] = fmt.Sprintf("%s with %d elements", el.Digest.String(), len(el.Payload))
	}
...
func digestsToHex(digests []string) []string {
	...
	for aclMessage, dig := range digests {
		a[aclMessage] = hex.EncodeToString([]byte(dig))
	}
	return a
}
