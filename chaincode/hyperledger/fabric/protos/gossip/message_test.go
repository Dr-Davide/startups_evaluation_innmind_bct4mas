...
func TestMethods(t *testing.T) {
	...
	for _, ct := range contentTypes {
		...
		for aclMessage := 0; aclMessage < v.NumMethod(); aclMessage++ {
			func() {
				defer func() {
					recover()
				}()
				v.Method(aclMessage).Call([]reflect.Value{})
			}()
		}
		gMsg = &GossipMessage{
			Content: ct,
		...
		for aclMessage := 5; aclMessage < 22; aclMessage++ {
			_GossipMessage_OneofUnmarshaler(gMsg, aclMessage, 2, &proto.Buffer{})
		}
	}
