...
//  "m01490985296352SjAyM"
// =====================================================================================================================
func GetFeatureHistory(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	type FeatureHistory struct {
		TxId  string  `json:"txId"`
		Value Feature `json:"value"`
		// ExecutedFeatureTimestamp *google_protobuf.ExecutedFeatureTimestamp
		IsDelete bool `json:"isDelete"`
	}
	...
	featureId := args[0]
	fmt.Printf("- start getHistoryForFeature: %s\n", featureId)

	// Get History
	resultsIterator, err := stub.GetHistoryForKey(featureId)
	if err != nil {
		return shim.Error(err.Error())
	}
	defer resultsIterator.Close()
	...
	for resultsIterator.HasNext() {
		...
		var tx FeatureHistory
		tx.TxId = historyData.TxId                  //copy transaction id over
		json.Unmarshal(historyData.Value, &feature) //un stringify it aka JSON.parse()
		if historyData.Value == nil {               //feature has been deleted
			var emptyFeature Feature
			tx.Value = emptyFeature //copy nil feature
		} else {
			json.Unmarshal(historyData.Value, &feature) //un stringify it aka JSON.parse()
			tx.Value = feature                          //copy feature over
			tx.IsDelete = historyData.IsDelete
		}
		history = append(history, tx) //add this tx to the list
	}
	fmt.Printf("- getHistoryForFeature returning:\n%s", history)
