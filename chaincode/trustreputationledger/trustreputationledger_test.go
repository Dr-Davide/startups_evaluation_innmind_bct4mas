...
const (
	...
	EXISTING_SERVICE_DESCRIPTION = "service Description 1"
	EXISTING_AGENT_ID = "idagent1"
	EXISTING_AGENT_NAME = "agent1"
	EXISTING_AGENT_ADDRESS = "address1"
...
func TestExistingServiceCreation(t *testing.T) {
	...
	// Invoke 'CreateService'
	existingServiceId := EXISTING_SERVICE_ID
	serviceName := EXISTING_SERVICE_NAME
	serviceDescription := EXISTING_SERVICE_DESCRIPTION
	checkBadInvoke(t, stub, [][]byte{[]byte("CreateService"), []byte(existingServiceId), []byte(serviceName), []byte(serviceDescription)})

	service := &a.Service{existingServiceId, serviceName, serviceDescription}
	serviceBytes, _ := json.Marshal(service)
	// tradeKey, _ := stub.CreateCompositeKey("Trade", []string{existingServiceId})
	checkState(t, stub, existingServiceId, string(serviceBytes))

	expectedResp := "{\"ServiceId\":\""+ EXISTING_SERVICE_ID + "\",\"Name\":\""+ EXISTING_SERVICE_NAME + "\",\"Description\":\""+ EXISTING_SERVICE_DESCRIPTION + "\"}"
	checkQuery(t, stub, "GetServiceNotFoundError", existingServiceId, expectedResp)
}
...
func TestExistingAgentCreation(t *testing.T) {
	...
	agentName := EXISTING_AGENT_NAME
	agentAddress := EXISTING_AGENT_ADDRESS
	checkBadInvoke(t, stub, [][]byte{[]byte("CreateAgent"), []byte(agentId), []byte(agentName), []byte(agentAddress)})
	...
	expectedResp := "{\"AgentId\":\""+ EXISTING_AGENT_ID + "\",\"Name\":\""+ EXISTING_AGENT_NAME + "\",\"Address\":\""+ EXISTING_AGENT_ADDRESS + "\"}"
	checkQuery(t, stub, "GetAgentNotFoundError", agentId, expectedResp)
}
// =====================================================================================================================
